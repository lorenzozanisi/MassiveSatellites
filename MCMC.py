...
class sampler:
    ...
    def run_chain(self,n_walkers, n_steps,reset = False, name='mcmc', filename='MCMC.h5'):
        ndim = 3
        ...
        with Pool() as pool:
            ...
            Sampler = emcee.EnsembleSampler(n_walkers, ndim, self.lnprob, pool=pool, backend=backend)
            start = time.time()
            Sampler.run_mcmc(walkers_positions, nsteps=n_steps,progress=True)#,skip_initial_state_check=True)
            end = time.time()
...
class postprocess:
    ...
    def check_model(self):
        ...
        maker = make_satellites_only(use_peak=True, use_Multidark=True)
